...
def individual_dataset(datadir, filename, indicator, units, datahub, data, codes):
    #wb = xlwt.Workbook(encoding='utf')
    fullpath = datadir + "/" + filename + 'x'
    wb = xlsxwriter.Workbook(fullpath)
    ...
    ws.write(0, 0, indicator)
    ws.write(1, 0, units)
    ws.write(2, 0, 'Code')
    ws.write(2, 1, 'Continent, Region, Country')
